...
async function main() {
  try {
    ...
    // 5a. Create vouchers for all customers
    console.log('Creating vouchers...');
    const voucherStatuses = ['active', 'used', 'expired'];
    const voucherCodes = [
      'VOUCHER123', 'REWARD456', 'FREE789', 'SAVE012', 'DEAL345',
      'OFFER678', 'BONUS901', 'GIFT234', 'SPECIAL567', 'VIP890',
      'PREMIUM123', 'EXCLUSIVE456', 'LIMITED789', 'FLASH012', 'QUICK345',
      'INSTANT678', 'DIRECT901', 'FAST234', 'RAPID567', 'SWIFT890'
    ];
    ...
    for (let i = 1; i <= 900; i++) {
      ...
      // Create 2 vouchers per customer
      for (let j = 1; j <= 2; j++) {
        const status = voucherStatuses[getRandomInt(0, voucherStatuses.length - 1)];
        const voucherCode = `${voucherCodes[getRandomInt(0, voucherCodes.length - 1)]}${i}${j}`;
        
        // Create redemption first (required for voucher)
        ...
        if (status === 'used') {
          usedAt = new Date(now.getTime() - getRandomInt(1, 30) * 24 * 60 * 60 * 1000); // Used 1-30 days ago
          expiresAt = new Date(now.getTime() + getRandomInt(1, 365) * 24 * 60 * 60 * 1000); // Expires in future
        } else if (status === 'expired') {
          expiresAt = new Date(now.getTime() - getRandomInt(1, 30) * 24 * 60 * 60 * 1000); // Expired 1-30 days ago
        } else {
          // active
          expiresAt = new Date(now.getTime() + getRandomInt(1, 365) * 24 * 60 * 60 * 1000); // Expires in future
        }
    ...
    console.log('Created 1800 vouchers (2 per customer)');
