  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data.error === 'PARTNER_EMAIL_VERIFICATION_REQUIRED') {
          router.push(`/auth/verify-email?userId=${userId}&email=${encodeURIComponent(email)}`);
          return;
        } else if (data.error === 'PARTNER_MOBILE_VERIFICATION_REQUIRED') {
          router.push(`/auth/verify-mobile?userId=${userId}&email=${encodeURIComponent(email)}`);
          return;
        } else if (data.error === 'ACCOUNT_UNDER_REVIEW' || data.error === 'PENDING_APPROVAL') {
      ...
      if (signInResult?.error) {
        ...
        if (signInResult.error === 'ACCOUNT_SUSPENDED' || signInResult.error === 'PENDING_APPROVAL') {
          router.push('/partner/pending-approval');
          return;
        }
      ...
      // Login successful - redirect to partner dashboard
            ...
            <div className="flex gap-2" onPaste={handlePaste}>
              {code.map((digit, index) => (
                <input
                  ...
                  className="w-12 h-14 text-center text-2xl font-semibold border-2 border-gray-300 rounded-xl focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 transition-colors"
