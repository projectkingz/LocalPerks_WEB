...
function VerifyMobileCustomerContent() {
  ...
  useEffect(() => {
    ...
    if (codeSentRef.current) {
      console.log('⏭️  Code already sent, skipping');
      return;
    }
    ...
    // Code was already sent during registration
    codeSentRef.current = true;
    console.log('✅ Using code sent during registration');
  }, [userId, email]);
  ...
  const handleResendCode = async () => {
    ...
    try {
      const response = await fetch('/api/auth/resend-verification', {
        ...
        body: JSON.stringify({ userId, email, method: 'whatsapp' }),
      });
      ...
      >
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-gray-600">
            Didn't receive a code? Check your WhatsApp or try resending.
          </p>
        </div>
      </motion.div>
    </div>
...
export default function VerifyMobileCustomerPage() {
  return (
    <Suspense fallback={
